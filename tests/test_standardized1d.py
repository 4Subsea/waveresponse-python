...
class Test_Torsethaugen:
    ...
    def test__gamma(self, hs_i, tp_i, gamma_expected):
        ...
        assert rs_out == pytest.approx(gamma_expected)
    ...
    def test_reference_case(self):
        """
        Integration test. The reference is manually compared to wave spectrum
        generated by OrcaFlex. Negligible differences where discovered and attributed
        to round-off errors. ``waveresponse`` utilizes the parameter precission
        ...
        spectrum = wr.Torsethaugen(freq_expected, freq_hz=True)
