...
- :math:`T_{pj}` is the peak period for wave component :math:`j`.
- :math:`q_i` is a shape factor for wave component :math:`j`.
...
The full Och-Hubble spectrum can be generated by adding together values from the
two wave components:

.. code:: python

    freq, vals_swell = spectrum(hs_swell, tp_swell, q_swell)
    freq, vals_wind = spectrum(hs_wind, tp_wind, q_wind)

    vals_tot = vals_swell + vals_wind


Ochi-Hubble spectra new
-----------------------
